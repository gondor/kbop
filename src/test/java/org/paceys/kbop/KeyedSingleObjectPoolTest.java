...
public class KeyedSingleObjectPoolTest extends AbstractPoolTest<IKeyedObjectPool.Single<String, String>> {
	...
	try
	{
	  ...
	  Future<?> f = es.submit(new Runnable() {
		public void run() {
		  try
		  {
			IPooledObject<String> obj = pool().borrow(POOL_KEY, 500, TimeUnit.MILLISECONDS);
			obj.release();
	  ...
	  // Test that the main thread can still acquire the object since it owns the contract and hasn't releasd it yet
  ...
  static class TestLifecycleFactory implements IPoolObjectFactory<String, Boolean> 
  {
	int lifecycleCount;

	public Boolean create(PoolKey<String> key) { lifecycleCount++; return true; }
	public void activate(Boolean object) { lifecycleCount++; }
	public void passivate(Boolean object) { lifecycleCount++; }
	public void destroy(Boolean object) { lifecycleCount++; }
  }
}
